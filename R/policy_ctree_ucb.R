#'ctreeucb agorithm
#'
#'ctreeucb automatically create homogeneous groups by a conditional inference
#'method (see  \code{\link{ctree}}) in a collection and processing step before
#'the A/B test (step 1). These groups are created according to the objective of
#'the test using information from previous items (obtained rewards, items
#'characteristics, temporal information, \ldots). This information comes from
#'the items that have already been subjected to the original variation (A),
#'implemented before the test. In the A/B test period (step 2), the method
#'defines as many non-contextual bandits  (see  \code{\link{UCB}}) as there are
#'groups. Each bandit aims to find the optimal variation associated to its
#'group. So, a new item is firstly classed into a group and then the associated
#'bandit chooses the variation to which the item must be affected.
#'
...
ctreeucb <- function(dt,visitor_reward,K=ncol(visitor_reward), ctree_parameters_control=ctreeucb_parameters_control_default(dt,visitor_reward)){
 ...
 # set.seed(4321)
  ...
  for(i in levels(as.factor(dt$groups ))){
    ...
    #UCB results
    ucb_temp_res <- UCB(visitor_reward=visitor_reward_for_ctree , K=K ,alpha =ctree_parameters_control$alpha)
