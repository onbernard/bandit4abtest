...
#'
#'UCB algorithme with rejection sampling method. Exclud any choices which not
#'corresponds to real exepriments in dataset Stop if something is wrong.
#'Generate a matrix to save the results (S). \itemize{ At each iteration, \item
#'Calculates the arm probabilities, \item Choose the arm with the maximum upper
#'bound (with alpha parameter) \item Receives a reward in visitor_reward for the
#'arm and associated iteration \item Updates the results matrix S. } Returns the
#'calculation time. Review the estimated, actual averages and number of choices
#'for each arm. See also \code{\link{ConditionForUCB}},
#'\code{\link{GenerateMatrixS}}, \code{\link{ProbaMaxForUCB}} and
...
#'ucb_rejection_sampling_alloc<-UCB_rejection_sampling(visitor_reward,alpha= 0)
#'@import tictoc
#'@export
